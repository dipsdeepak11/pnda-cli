...
def _ensure_certs():
    ...
    exts = ['key', 'crt']
    if _has_certs(local_certs_path, exts):
        exts = ['key', 'crt', 'yaml']
        if _has_all_certs(local_certs_path, exts):
            CONSOLE.warning('All certificates are already present. Nothing to do.')
            return
        else:
...
def _has_all_certs(local_certs_path, exts):
    ret = True
    ...
    for role in roles:
        for ext in exts:
            ...
            if not files:
                CONSOLE.info('%s needs to be generated', file_name)
                ret = False
    return ret

def _has_certs(local_certs_path, exts):
    for ext in exts:
        files = glob.glob(os.path.join(local_certs_path, '*', '*.'+ext))
        if files:
            LOG.info('Security material was detected: %s', files[0])
            return True
    LOG.info('No leaf certificates were detected')
    return False
